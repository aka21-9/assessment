...
// 作ったけど、すぐに代入する無名関数というfunctionの()の前に名前がないから
assessmentButton.onclick = function() {
  const userName = userNameInput.value;
  //　名前が空の時は処理を実行しない
  if (userName.length === 0){
    // returnは終わらせる実行しないという意味も持つ
    return;
  }
  
// 診断結果表示エリアの作成
// 作成の前にタグを空にする　何回押しても子要素を全部削除したという意味になる。
resultDivision.innerText = '';
const header = document.createElement('h3'); //h3タグを作る
header.innerText = '診断結果'; //中身の文章　内側のテキストを作るdivの
resultDivision.appendChild(header); //divの子要素
